...
``{"image", "metadata", "task", "label"}``. Skips gracefully when the
...
def test_flood_sample_contract():
    ...
    assert len(dataset) > 0, "no .tif samples found under images/"

    sample = dataset[0]

    assert set(sample.keys()) == {"image", "metadata", "task", "label"}
